...
namespace wtv
{
		...
		m_creationParams(params)
	{
		...
		Init();
		m_gpuTimelineSemaphore = MakeRef<VulkanSemaphore>(this);
		m_debugNamer.Init(this);
	}
	...
	{
		m_allocator = MakeRef<VMAAllocator>(this, m_services);
		return true;
	}
	...
	RefPtr<IBinarySemaphore> VulkanDevice::CreateBinarySemaphore() const
	{
		return MakeRef<VulkanBinarySemaphore>(this);
	}
	...
	int VulkanDevice::GetMemoryTypeIndex(VkMemoryPropertyFlags memoryPropFlags)
	{
		for (int i = 0; i < m_memoryProperties.memoryProperties.memoryTypeCount; i++)
		{
			if ((m_memoryProperties.memoryProperties.memoryTypes[i].propertyFlags & memoryPropFlags) == memoryPropFlags)
			{
				return i;
			}
		}
	...
	RefPtr<IGPUBuffer> VulkanDevice::CreateBuffer(const IGPUBuffer::CreationParams& params, const std::string& name)
	{
